...
class Cloudflair:
    def __init__(self, censys_api_id, censys_api_secret):
        ...
        self.censys_client = censys_client.CensysClient(api_id=censys_api_id, api_secret=censys_api_secret)

    def run(self, domain, output_file):
        hosts = self.find_hosts(domain)
        self.print_hosts(hosts)
        if len(hosts) is 0:
            logging.info('The target is most likely not vulnerable.')
            return set()

        origins = self.find_origins(domain, hosts)
        ...
        self.print_origins(origins)
        self.save_origins_to_file(origins, output_file)
        return origins

    def find_hosts(self, domain):
        if not dns_utils.is_valid_domain(domain):
            raise ValueError(f'The domain "{domain}" looks invalid.')

        if not self.cloudflare.uses_cloudflare(domain):
        ...
        cert_fingerprints = self.censys_client.get_certificates(domain)
        ...
        hosts = self.censys_client.get_hosts(cert_fingerprints)
        hosts = [host for host in hosts if not self.cloudflare.is_cloudflare_ip(host)]
    ...
    def find_origins(self, domain, candidates):
        logging.info('Testing candidate origin servers')
        original_response = self.retrieve_original_page(domain)
        ...
        for host in candidates:
            try:
                ...
                response = requests.get(url=url, timeout=config['http_timeout_seconds'], headers=headers, verify=False)
            ...
            if len(response.text) > 0:
                ...
                if page_similarity > config['response_similarity_threshold']:
                    origins.append((host, 'HTML content is %d %% structurally similar to %s' % (
    ...
    def retrieve_original_page(domain):
        ...
        try:
            ...
            raise RuntimeError(f'{url} timed out after {config["http_timeout_seconds"]} seconds.')
        except requests.exceptions.RequestException as e:
            raise RuntimeError('Failed to retrieve %s' % url)

        status_code = original_response.status_code
        if status_code != 200:
            raise RuntimeError(f'{url} responded with an unexpected HTTP status code {status_code}')
...
if __name__ == "__main__":
    ...
    Cloudflair(censys_api_id, censys_api_secret).run(args.domain, args.output_file)
